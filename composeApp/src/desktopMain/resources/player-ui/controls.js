...
window.playerUpdate = update => {
  ...
  window.clipUi?.syncPlayback(durationMs, positionMs);
...
document.addEventListener("keydown", event => {
  ...
  // StreamCut: clip keys (I/O mark, ,/. step, A/B/X/R, /, ?) come before the
  // single-key shortcuts upstream keeps adding, several of which reuse them
  // (I/O subtitle opacity, / speed reset, A, B). They only claim a key while a
  // clip is being cut, so every other player keeps upstream's meaning.
  if (!activeModal && window.clipUi?.handleKey?.(event)) return;
