...
internal expect object ClipExtractor {
     ...
     * determined.
     *
     * The trim UI steps the playhead one frame at a time (`,` / `.`), which needs
     * the source's real rate: a fixed guess is off by a whole frame every few
     * presses on anything that is not 24fps. The native player does not expose a
     * rate, so this is probed from the container instead. Suspending because it
     * reaches the network -- for a remote source it is a ranged read of the header.
     */
